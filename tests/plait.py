...
def pre_stitch_ast(source: str) -> dict:
    return json.loads(
        pf.convert_text(
            knitty_preprosess(source), input_format="markdown", output_format="json"
        )
    )
...
def as_json(document):
    ...
    return json.loads(
        pf.convert_text(document, output_format="json", input_format="markdown")
    )
...
def code_block(request):
    if request.param == "python":
        ...
        code = "f <- function(x){\n  return(x * 2)\n}\n\nf(2)"
    block = {"t": "CodeBlock", "c": [["", ["{}".format(request.param)], []], code]}
    return block
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                [
                    {
                        "content": {"data": {}, "execution_count": 4},
...
class TestIntegration:
    def test_from_file(self, document_path, clean_stdout):
        with open(document_path, "r", encoding="utf-8") as f:
            R.Stitch("stdout", "html", self_contained=False).stitch_ast(
                pre_stitch_ast(f.read())
            )

    def test_from_source(self, document, clean_stdout):
        R.Stitch("stdout", "html", self_contained=False).stitch_ast(
            pre_stitch_ast(document)
        )
    ...
    def test_image_chunkname(self):
        ...
        result = R.Stitch("foo", "latex", standalone=False).stitch_ast(
            pre_stitch_ast(code)
        )
        blocks = result["blocks"]
    ...
    def test_image_attrs(self):
        ...
        result = R.Stitch("foo", "html", standalone=False).stitch_ast(
            pre_stitch_ast(code)
        )
        blocks = result["blocks"]
    ...
    def test_image_no_self_contained(self, clean_python_kernel, clean_name):
        ...
        blocks = result["blocks"]
        expected = (
            "{}_files/unnamed_chunk_0.png".format(clean_name),
    ...
    def test_image_no_self_contained_formats(
        self, clean_python_kernel, clean_name, fmt
    ):
        code = dedent(
            """\
        ```{{python}}
